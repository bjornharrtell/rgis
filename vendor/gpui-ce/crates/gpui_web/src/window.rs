...
pub(crate) struct WebWindowInner {
    ...
    pub(crate) gpu_context: (Arc<wgpu::Device>, Arc<wgpu::Queue>),
...
impl WebWindow {
    ...
    pub(crate) fn new(
        ...
        let inner = Rc::new(WebWindowInner {
            browser_window,
            canvas,
            gpu_context: (Arc::clone(&context.device), Arc::clone(&context.queue)),
            ime_mirror,
...
impl PlatformWindow for WebWindow {
    ...
    fn gpu_context(&self) -> Option<Box<dyn std::any::Any>> {
        Some(Box::new((
            Arc::clone(&self.inner.gpu_context.0),
            Arc::clone(&self.inner.gpu_context.1),
        )))
    }
